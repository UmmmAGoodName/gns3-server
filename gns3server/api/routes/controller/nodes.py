...
async def spice_console(
    ...
    compute = node.compute
    log.info(
        f"New client {websocket.client.host}:{websocket.client.port} has connected to controller SPICE console WebSocket"
    )
    ...
    spice_console_compute_url = (
        f"{websocket.url.scheme}://{compute_host}:{compute.port}/v3/compute/projects/"
        f"{node.project.id}/{node.node_type}/nodes/{node.id}/console/spice"
    )

    async def spice_receive(spice_console_compute):
        """
        Receive binary WebSocket data from client and forward to compute SPICE console WebSocket.
        """
        ...
        try:
            ...
            await spice_console_compute.close()
            log.info(
                f"Client {websocket.client.host}:{websocket.client.port} has disconnected from controller"
    ...
    try:
        # receive binary data from compute SPICE console WebSocket and forward to client
        log.info(f"Forwarding SPICE console WebSocket to '{spice_console_compute_url}'")
        server_config = Config.instance().settings.Server
        user = server_config.compute_username
        password = server_config.compute_password
        if not user:
        ...
        if user and password:
            ...
            auth = aiohttp.BasicAuth(user, "")
        ssl_context = Controller.instance().ssl_context()
        async with HTTPClient.get_client().ws_connect(spice_console_compute_url, auth=auth, ssl_context=ssl_context) as ws:
        ...
        log.error(f"Client error received when forwarding to compute SPICE console WebSocket: {e}")
